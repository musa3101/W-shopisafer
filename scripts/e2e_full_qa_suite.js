...
async function runFullE2ESuite() {
  console.log(
    "🚀 Iniciando Suite de Pruebas E2E de Calidad Final — Isafer Boutique...\n",
  );
  ...
  try {
    ...
    // Rellenar formulario
    await page.fill(
      'input[placeholder="Ej: Vestido Barbie Luxe Pink"]',
      testProductName,
    );
    ...
    await page.fill(
      "textarea",
      "Producto creado automáticamente para pruebas E2E.",
    );
    ...
    try {
      ...
      const dbResRaw = execSync(
        `npx -y @insforge/cli db query "${dbCheckSql}" --json 2>&1`,
        { encoding: "utf-8" },
      );
      const dbRes = JSON.parse(dbResRaw);
      if (dbRes.rows && dbRes.rows.length > 0) {
        console.log(
          `  ✅ Producto verificado en PostgreSQL DB! (ID: ${dbRes.rows[0].id}, Precio: $${dbRes.rows[0].price}, Stock: ${dbRes.rows[0].stock})`,
        );
      } else {
    ...
    if (await productOnWeb.isVisible().catch(() => false)) {
      console.log(
        "  ✅ ¡ÉXITO! El producto creado por Camila aparece INMEDIATAMENTE en la Web Pública!",
      );
    ...
    const editBtn = page
      .locator(
        `tr:has-text("${testProductName}") button, div:has-text("${testProductName}") button`,
      )
      .first();
    ...
    const deleteBtn = page
      .locator(
        `tr:has-text("${testProductName}") button[title*="Eliminar"], tr:has-text("${testProductName}") button:has-text("Borrar"), button:has(.lucide-trash)`,
      )
      .first();
    if (await deleteBtn.isVisible().catch(() => false)) {
      ...
      const confirmDelete = page
        .locator(
          'button:has-text("Eliminar"), button:has-text("Sí"), button:has-text("Confirmar")',
        )
        .last();
    ...
    try {
      execSync(
        `npx -y @insforge/cli db query "DELETE FROM products WHERE name LIKE 'E2E TEST%'" --json 2>&1`,
      );
      console.log(
        "  🧹 Limpieza de registros de prueba completada en PostgreSQL.",
      );
    } catch (e) {}
    ...
    const addCartBtn = clientPage
      .locator(
        'button:has-text("Añadir"), button:has-text("Bolsa"), button:has(.lucide-shopping-bag)',
      )
      .first();
    ...
    if (await dbConnectedTag.isVisible().catch(() => false)) {
      console.log(
        "  ✅ El panel de diagnóstico muestra: Base de datos PostgreSQL — CONECTADO!",
      );
  ...
  console.log(
    `- Crear Producto Admin: ${results.createProduct ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Verificación Producto en Web: ${results.verifyProductWeb ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Edición de Producto: ${results.updateProduct ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Eliminación de Producto: ${results.deleteProduct ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Flujo Carrito Cliente: ${results.createOrderWeb ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Diagnóstico DB en Admin: ${results.dbHealthCheck ? "PASS 🟢" : "FAIL 🔴"}`,
  );
  console.log(
    `- Errores Consola Críticos: ${consoleErrors.length === 0 ? "Ninguno 🟢" : `${consoleErrors.length} detectados 🟡`}`,
  );
  console.log(
    `- Solicitudes HTTP Fallidas: ${failedRequests.length === 0 ? "Ninguna 🟢" : `${failedRequests.length} detectadas 🟡`}`,
  );
  console.log("==========================================\n");
}
