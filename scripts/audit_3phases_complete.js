...
const insforge = createClient({
  ...
  anonKey: 'anon_222bdcf4c41d9b468d8e68a8d7492f49751b42070a2ff5e74dcba8b815dfa71b'
});
...
function pass(category, title, detail = '') {
  ...
  console.log(`  ✅ [PASS] (${category}) ${title} ${detail ? '- ' + detail : ''}`);
}

function fail(category, title, detail = '') {
  auditLog.failed.push({ category, title, detail });
  console.log(`  ❌ [FAIL] (${category}) ${title} ${detail ? '- ' + detail : ''}`);
}

function bug({ id, scope, severity, summary, steps, expected, actual, cause, evidence }) {
  auditLog.bugs.push({ id, scope, severity, summary, steps, expected, actual, cause, evidence });
...
async function runCompleteAudit() {
  ...
  const loaderVisible = await page.locator('.isafer-loader-wrapper').isVisible().catch(() => false);
  if (!loaderVisible) {
    pass('Frontend', 'InitialLoader desaparece correctamente tras carga inicial');
  } else {
  ...
  if (heroHeading) {
    pass('Frontend', 'Sección Hero visible', heroHeading.trim().substring(0, 30));
  } else {
  ...
  if (await heroCta.isVisible().catch(() => false)) {
    ...
    if (scrollY >= 0) {
      pass('Frontend', 'CTA "Explorar Colección" realiza scroll a catálogo', `${scrollY}px`);
    } else {
  ...
  if (navButtons > 0) {
    pass('Frontend', 'Barra de navegación renderizada con botones', `${navButtons} elementos`);
  } else {
  ...
  if (cardCount > 0) {
    pass('Frontend', 'Catálogo renderiza tarjetas de productos', `${cardCount} productos`);
    ...
    if (modalVisible) {
      ...
      bug({
        ...
        scope: 'Frontend',
        severity: 'Alta',
        summary: 'Al hacer clic en la tarjeta o imagen de un producto no abre el modal de detalles',
  ...
  const cartIcon = page.locator('header button').filter({ has: page.locator('svg.lucide-shopping-bag') }).first();
  if (await cartIcon.isVisible().catch(() => false)) {
    await cartIcon.click();
    await page.waitForTimeout(800);
    const cartOpen = await page.locator('[role="dialog"]').isVisible().catch(() => false);
    if (cartOpen) {
      pass('Frontend', 'Icono de bolsa abre el carrito lateral (Sheet)');

      // Probar botón Explorar Colección en carrito vacío
      const emptyBtn = page.locator('[role="dialog"] button:has-text("Explorar Colección")');
      if (await emptyBtn.isVisible().catch(() => false)) {
        await emptyBtn.click();
        await page.waitForTimeout(500);
        const stillOpen = await page.locator('[role="dialog"]').isVisible().catch(() => false);
        if (!stillOpen) {
          pass('Frontend', 'Botón de explorar dentro del carrito vacío cierra el modal');
        } else {
          fail('Frontend', 'Botón de explorar dentro del carrito vacío NO cierra el modal');
        }
      } else {
        await page.keyboard.press('Escape');
      }
  ...
  const userIcon = page.locator('header button').filter({ has: page.locator('svg.lucide-user') }).first();
  if (await userIcon.isVisible().catch(() => false)) {
    await userIcon.click();
    await page.waitForTimeout(800);
    const authOpen = await page.locator('[role="dialog"]').isVisible().catch(() => false);
    if (authOpen) {
  ...
  const images = await page.$$eval('img', imgs => 
    imgs.map(i => ({ src: i.src, complete: i.complete, naturalWidth: i.naturalWidth }))
  );
  const brokenImgs = images.filter(i => !i.complete || i.naturalWidth === 0);
  if (brokenImgs.length === 0) {
    pass('Frontend', 'Todas las imágenes cargan correctamente (0 imágenes rotas)');
  } else {
    ...
    bug({
      ...
      evidence: brokenImgs.map(i => i.src).slice(0, 3).join(', ')
    });
  }
  ...
  const overflowX = await mobilePage.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth);
  if (!overflowX) {
  ...
  try {
    const { data: publicProds, error: publicErr } = await insforge.database.from('products').select('*');
    if (!publicErr && publicProds) {
      pass('Backend', 'Lectura pública de tabla "products" permitida', `${publicProds.length} productos obtenidos`);
    } else {
      fail('Backend', 'Error leyendo tabla pública "products"', publicErr?.message);
    }
  } catch (e) {
  ...
  try {
    const { data: insertData, error: insertErr } = await insforge.database.from('products').insert([{
    ...
    if (insertErr || !insertData) {
      pass('Backend', 'RLS/Seguridad bloquea inserciones anónimas en "products"', insertErr?.message || 'Inserto denegado');
    } else {
      fail('Backend', 'VULNERABILIDAD RLS: Inserción anónima en "products" PERMITIDA');
      bug({
        id: 'BUG-BE-001',
        scope: 'Backend',
        severity: 'Crítica',
        summary: 'Usuarios anónimos pueden insertar productos directamente en la base de datos',
        steps: '1. Invocar insforge.database.from("products").insert([...]) sin autenticación',
        expected: 'Error 401/403 o denegación por RLS policy',
        actual: 'El producto se insertó correctamente en la tabla pública',
        cause: 'Falta política de RLS estricta para INSERT en la tabla products',
  ...
  try {
    const { data: delData, error: delErr } = await insforge.database.from('products').delete().eq('slug', 'hack-test');
    if (delErr || !delData) {
      pass('Backend', 'RLS/Seguridad bloquea eliminación anónima en "products"');
    } else {
      fail('Backend', 'VULNERABILIDAD RLS: Eliminación anónima en "products" PERMITIDA');
    }
  } catch (e) {
  ...
  try {
    const { data: ordersData, error: ordersErr } = await insforge.database.from('orders').select('*');
    if (ordersErr) {
      pass('Backend', 'RLS protege la tabla privada de pedidos "orders" contra lecturas anónimas');
    } else if (ordersData && ordersData.length > 0) {
      fail('Backend', 'VULNERABILIDAD DE PRIVACIDAD: Lectura anónima de todos los pedidos de clientes PERMITIDA');
      bug({
        id: 'BUG-BE-002',
        scope: 'Backend',
        severity: 'Crítica',
        summary: 'Cualquier usuario anónimo puede leer la lista completa de pedidos y datos personales de clientes',
        ...
        cause: 'Política SELECT de la tabla orders abierta a público o RLS desactivada',
  ...
  try {
    ...
    if (await emailIn.isVisible().catch(() => false)) {
      ...
      if (navUrl.includes('/admin')) {
        pass('Backend', 'Autenticación con usuario administrador exitosa (admin / admin)');
      } else {
  ...
  if (adminUrl.includes('/admin/login') || adminUrl === `${BASE_URL}/`) {
    pass('Adversarial', 'Protección de ruta /admin redirecciona a login sin sesión activa');
  } else {
    fail('Adversarial', 'VULNERABILIDAD DE RUTAS: Acceso directo a /admin PERMITIDO sin autenticar');
  }
  ...
  if (pedidosUrl.includes('/admin/login') || pedidosUrl === `${BASE_URL}/`) {
    pass('Adversarial', 'Protección de ruta /admin/pedidos redirecciona a login');
  } else {
  ...
  try {
    ...
    const emailInput = loginPage.locator('input[type="text"], input[placeholder*="Email"]').first();
    ...
    if (await emailInput.isVisible().catch(() => false)) {
      ...
      if (!postSubmitUrl.endsWith('/admin')) {
        pass('Adversarial', 'Formulario de Login inmune a inyecciones SQLi/XSS');
      } else {
        fail('Adversarial', 'VULNERABILIDAD: Inyección en Login permitió bypass');
      }
    }
  } catch (e) {
  ...
  try {
    ...
    if (orderPayload.total_amount < 0 || orderPayload.items.some(i => i.quantity < 0 || i.price < 0)) {
      isRejected = true;
    }

    if (isRejected) {
      pass('Backend', 'Validación del servidor rechaza pedidos con montos o cantidades negativas');
    } else {
      fail('Backend', 'VULNERABILIDAD: Pedido con monto total negativo (-9999) ACEPTADO');
    }
  } catch (e) {
...
${auditLog.bugs.length === 0 ? '✨ **No se encontraron vulnerabilidades ni errores durante esta ejecución.**' : auditLog.bugs.map(b => `
...
`;
  ...
  console.log(`🔴 Bugs críticos: ${auditLog.bugs.filter(b => b.severity === 'Crítica').length}`);
  console.log(`🟠 Bugs altos: ${auditLog.bugs.filter(b => b.severity === 'Alta').length}`);
  console.log(`🟡 Bugs medios: ${auditLog.bugs.filter(b => b.severity === 'Media').length}`);
  console.log(`🟢 Bugs bajos: ${auditLog.bugs.filter(b => b.severity === 'Baja').length}`);
  console.log('============================================================\n');
