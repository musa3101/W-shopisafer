...
async function main() {
  ...
  try {
    const acceptBtn = await page.$(
      'button[aria-label*="Aceptar"], button:has-text("Aceptar todo")',
    );
    if (acceptBtn) {
  ...
  const hoursBtn = await page.$(
    'button[data-item-id*="oh"], [aria-label*="Abierto"], [aria-label*="horario"], button:has-text("Cierre:")',
  );
  if (hoursBtn) {
  ...
  const hoursData = await page.evaluate(() => {
    const rows = Array.from(
      document.querySelectorAll('tr, [role="row"], table'),
    );
