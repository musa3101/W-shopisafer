...
async function checkI18nAndResponsive() {
  ...
  for (const vp of viewports) {
    ...
    if (!hasHorizontalScroll) {
      console.log(
        `  ✅ [PASS] Responsive en ${vp.name}: Sin desbordamiento horizontal (Scroll: ${scrollWidth}px / Client: ${clientWidth}px)`,
      );
    } else {
      console.log(
        `  ❌ [FAIL] Responsive en ${vp.name}: Desbordamiento horizontal detectado (${scrollWidth}px > ${clientWidth}px)`,
      );
    }
  ...
  const langToggle = page
    .locator(
      'button:has-text("ES"), button:has-text("EN"), button[aria-label*="idioma"], button[aria-label*="Language"]',
    )
    .first();
  if (await langToggle.isVisible().catch(() => false)) {
    const currentLangText = await langToggle.textContent();
    console.log(
      `  ✅ [PASS] Conmutador de idioma visible en cabecera: "${currentLangText?.trim()}"`,
    );
    ...
    console.log(
      `  ✅ [PASS] Cambio de idioma ejecutado. Nuevo estado: "${newLangText?.trim()}"`,
    );
  } else {
  ...
  const hasSpanishTerms =
    bodyText.includes("Colección") ||
    bodyText.includes("Boutique") ||
    bodyText.includes("Añadir");
  if (hasSpanishTerms) {
    console.log(
      "  ✅ [PASS] Textos principales en Español validados correctamente",
    );
  } else {
