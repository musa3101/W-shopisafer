...
async function checkRLS() {
  ...
  console.log('✅ Autenticado correctamente con ID de usuario:', authData.user.id);
  ...
  if (prodError) {
    ...
    console.log(`✅ Lectura de products exitosa! Se obtuvieron ${products.length} productos.`);
  }
  ...
  if (orderError) {
    ...
    console.log(`✅ Lectura de orders exitosa! Se obtuvieron ${orders.length} órdenes.`);
  }
  ...
  if (products && products.length > 0) {
    ...
    if (updateError) {
      console.error('❌ Error de RLS al actualizar producto:', updateError.message);
    } else {
      console.log('✅ Actualización de producto exitosa! Las políticas RLS de admin funcionan.');
    }
  } else {
