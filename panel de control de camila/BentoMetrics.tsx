...
export function BentoMetrics({ products, orders }: BentoMetricsProps) {
  const [timeRange, setTimeRange] = useState<"7days" | "30days" | "monthly">(
    "7days",
  );
  ...
  const stripeOrders = validOrders.filter(
    (o) => o?.stripe_session_id && o?.stripe_session_id !== "pending_session",
  );
  ...
  const maxOrderValue = validOrders.reduce(
  ...
  const handleExportCSV = () => {
    const cols: ExportColumn<BackendOrder>[] = [
      ...
      {
        header: "Monto USD",
        accessor: (o) => (Number(o.total_amount) || 0).toFixed(2),
      },
      {
        header: "Fecha",
        accessor: (o) =>
  ...
  const handleExportPDF = () => {
    const cols: ExportColumn<BackendOrder>[] = [
      ...
      {
        header: "Monto",
        accessor: (o) => `$${(Number(o.total_amount) || 0).toFixed(2)} USD`,
      },
    ];
    printReport(
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">
              Rendimiento en tiempo real e inteligencia comercial para Camila.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 auto-rows-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-10">
            <div>
              ...
              <div className="flex items-baseline gap-2 mt-2">
                <span className="text-3xl sm:text-4xl font-black text-rose-500 font-mono">
                  $
                </span>
        ...
        <div className="rounded-3xl bg-zinc-900 border border-zinc-800 p-6 flex flex-col justify-between relative overflow-hidden group hover:border-rose-650/30 transition-all duration-300 shadow-xl">
          <div className="flex items-center justify-between z-10 relative">
            ...
            <span className="text-[10px] font-black text-rose-450 uppercase tracking-widest">
              INVENTARIO CRÍTICO
            </span>
          </div>
          ...
          <div className="mt-6 z-10">
            <p className="text-xs text-zinc-400 font-medium">
              Prendas con Stock Bajo (&lt; 5 u.)
            </p>
            <h4
              className={`text-4xl font-black font-mono mt-1 tracking-tight ${lowStockCount > 0 ? "text-rose-500" : "text-emerald-450"}`}
            >
              {lowStockCount}
            </h4>
            <p className="text-xs text-zinc-500 mt-2 font-medium">
              {lowStockCount > 0
                ? "Requiere reabastecimiento en catálogo"
                : "Todo el inventario en estado óptimo"}
            </p>
          </div>
        </div>
