...
export function OrderManager({ orders, onOrderUpdated }: OrderManagerProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    "all" | "pending" | "processing" | "shipped_delivered" | "cancelled"
  >("all");
  ...
  const getWhatsAppLink = (order: BackendOrder) => {
    ...
    if (order.status === "pending")
      statusText = "está pendiente de confirmación";
    else if (order.status === "processing")
      statusText = "ya está siendo procesado";
    else if (order.status === "shipped") statusText = "ha sido enviado";

    const items = Array.isArray(order.items)
      ? order.items
          .map((i) => `${i.name || "Prenda"} (x${i.quantity || 1})`)
          .join(", ")
      : "";
  ...
  const filteredOrders = safeOrders.filter((o) => {
    ...
    if (activeTab === "processing")
      return matchesSearch && o.status === "processing";
    if (activeTab === "shipped_delivered")
      return (
        matchesSearch && (o.status === "shipped" || o.status === "delivered")
      );
      ...
      <div className="space-y-3">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => {
            ...
            // Status style maps
            let statusBadge =
              "bg-amber-450/10 border-amber-450/20 text-amber-500";
                ...
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 border-t border-zinc-850/60 pt-3 sm:pt-0 sm:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[10px] font-bold uppercase tracking-wider ${statusBadge}`}
                      >
                        ...
                        <div className="space-y-2 text-xs">
                          ...
                          <div className="flex items-start gap-2 text-zinc-350">
                            ...
                            <span className="leading-relaxed">
                              {order.shipping_address ||
                                "Retiro en boutique / Sin dirección"}
                            </span>
                          </div>
                          ...
                          {order.stripe_session_id && (
                            <div className="flex items-center gap-2 text-[10px] text-zinc-500 pt-1 border-t border-zinc-800/50 mt-2">
                              <span>
                                Pasarela: 💳 Tarjeta (Stripe Checkout)
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="bg-zinc-900/80 border border-zinc-850 p-4 rounded-2xl space-y-3 shadow-inner">
                        ...
                        <div className="space-y-2 max-h-36 overflow-y-auto">
                          {itemsList.map((item, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between items-center text-xs py-1.5 border-b border-zinc-850/60 last:border-0"
                            >
                              ...
                              <div className="text-right flex-shrink-0 font-mono">
                                ...
                                <span className="font-black text-rose-450 ml-3">
