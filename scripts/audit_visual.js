...
async function runAudit() {
  try {
    ...
    const menuButton = await mobilePage.locator('button:has(svg.lucide-menu), button[aria-label*="menu"], svg.lucide-menu').first();
    if (await menuButton.count() > 0) {
      ...
      // Cerrar menú móvil
      const closeButton = await mobilePage.locator('button:has(svg.lucide-x), svg.lucide-x').first();
    ...
    await browser.close();
    console.log('✓ Visual audit completed successfully! Screenshots saved in: ', BASE_DIR);
  } catch (error) {
