...
(async () => {
  ...
  const page = defaultContext ? defaultContext.pages()[0] || await defaultContext.newPage() : await browser.newPage();

  let loaded = false;
  for (const url of ['http://localhost:3000', 'http://localhost:5173', 'http://localhost:5174']) {
    try {
  ...
  const screenshotPath = path.join(__dirname, '..', 'carpeta de referencia', 'product_modal_preview.png');
