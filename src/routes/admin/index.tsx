...
function AdminDashboard() {
  ...
  const validOrders = activeOrders.filter(
  ...
  const activityFeed = [
    ...(orders || []).slice(0, 4).map((o) => ({
      ...
      title:
        o.status === "delivered"
          ? `Pedido #${o.id.slice(0, 6)} Entregado`
          : `Nuevo pedido recibido (#${o.id.slice(0, 6)})`,
      time: o.created_at
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <div className="mt-5 flex items-baseline justify-between">
            <div>
              <p
                className={`text-3xl sm:text-4xl font-black tracking-tight font-mono ${lowStockCount + outOfStockCount > 0 ? "text-rose-600" : "text-zinc-900"}`}
              >
            ...
            <div className="px-6 py-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/40">
              <div>
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  Pedidos registrados en PostgreSQL de la boutique
                </p>
              </div>
              <Link
                to="/admin/pedidos"
              ...
              {recentOrders.length === 0 ? (
                <div className="p-12 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-xs text-zinc-500 max-w-sm mt-1 mb-5">
                    No hay pedidos antiguos. Realiza una nueva compra desde la
                    tienda para ver cómo se registra en tiempo real.
                  </p>
                  <a
                    href="/"
                    target="_blank"
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-zinc-50/80 text-zinc-400 text-[10px] uppercase tracking-widest font-black">
                      <th className="px-6 py-3 border-b border-zinc-100">
                        ID Orden
                      </th>
                      <th className="px-6 py-3 border-b border-zinc-100">
                        Cliente
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-200/80 space-y-4">
            ...
            <div className="space-y-3.5">
              {activityFeed.map((item) => (
                <div
                  key={item.id}
                  className="flex items-start gap-3 p-2.5 rounded-2xl hover:bg-zinc-50 transition-colors"
                >
                  <div
