...
try {
  ...
  console.warn(
    "Advertencia: No se pudo leer .env.local, se intentará usar process.env",
  );
}
...
async function runAudit() {
  ...
  for (const page of pages) {
    try {
      ...
      if (res.ok) {
        report.push(
          `- **${page.name}** (${page.path}): ✓ **OK (HTTP ${res.status})** en ${duration}ms`,
        );
      } else {
        report.push(
          `- **${page.name}** (${page.path}): ❌ **FALLÓ (HTTP ${res.status})**`,
        );
        totalErrors++;
      }
    } catch (err) {
      report.push(
        `- **${page.name}** (${page.path}): ❌ **ERROR DE CONEXIÓN** (${err.message})`,
      );
      totalErrors++;
    }
  }
  ...
  try {
    ...
    if (!error && products) {
      report.push(
        `- **Catálogo de Productos**: ✓ **Conectado vía SDK** en ${duration}ms`,
      );
      ...
      for (const prod of products) {
        if (prod.images && prod.images.length > 0) {
          ...
          try {
            ...
            if (imgRes.ok) {
              report.push(
                `  - Prenda *"${prod.name}"*: ✓ Imagen en línea (HTTP 200)`,
              );
            } else {
              report.push(
                `  - Prenda *"${prod.name}"*: ❌ **IMAGEN ROTA (HTTP ${imgRes.status})** en URL: ${imgUrl}`,
              );
              brokenImages++;
              totalErrors++;
            }
            ...
            report.push(
              `  - Prenda *"${prod.name}"*: ❌ **IMAGEN INACCESIBLE** (Error de red) en URL: ${imgUrl}`,
            );
            brokenImages++;
            totalErrors++;
          }
          ...
          report.push(
            `  - Prenda *"${prod.name}"*: ⚠️ **Sin imagen de producto configurada**`,
          );
          totalWarnings++;
        }
      }
      ...
      report.push(
        `- **Catálogo de Productos**: ❌ **ERROR DE CONEXIÓN CON INSFORGE (SDK Error: ${error?.message || "Desconocido"})**`,
      );
      totalErrors++;
    }
  } catch (err) {
    report.push(
      `- **Conexión InsForge**: ❌ **ERROR DE RED AL CONECTAR AL BACKEND** (${err.message})`,
    );
    totalErrors++;
  }
  report.push("");
  ...
  for (const link of socialLinks) {
    try {
      ...
      if (res.status < 500) {
        report.push(
          `- **${link.name}** (${link.url}): ✓ Enlace vivo (HTTP ${res.status})`,
        );
      } else {
        report.push(
          `- **${link.name}** (${link.url}): ❌ **ALERTA (HTTP ${res.status})**`,
        );
        totalWarnings++;
      }
    } catch (e) {
      report.push(
        `- **${link.name}** (${link.url}): ❌ **ENLACE CAÍDO** (${e.message})`,
      );
      totalErrors++;
    }
  }
  ...
  console.log(
    "✓ Auditoría finalizada. Reporte guardado en docs/AUDIT_REPORT.md",
  );
}
