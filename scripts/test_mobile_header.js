...
async function run() {
  try {
    ...
    const searchBtnStatus = await page.evaluate(() => {
      ...
      const searchButton = Array.from(document.querySelectorAll('header button')).find(btn => {
        return btn.innerHTML.includes('lucide-search') || btn.getAttribute('aria-label') === 'Buscar productos';
      });
    ...
    console.log('Header Search Button mobile status:', JSON.stringify(searchBtnStatus, null, 2));
