...
const REPORT_DIR =
  "/Users/musa/Downloads/sopisafer/carpeta de referencia/qa_report";
...
async function runAudit() {
  ...
  if (brokenImages.length > 0) {
    recordIssue({
      ...
      expected:
        "Todas las imágenes deben cargar correctamente con naturalWidth > 0",
      actual: `Imágenes rotas: ${brokenImages
        .map((i) => i.src)
        .slice(0, 3)
        .join(", ")}`,
      severity: "Alta",
  ...
  if (await heroCta.isVisible().catch(() => false)) {
    ...
    if (scrollY < 50) {
      recordIssue({
        ...
        component: "Hero CTA",
        summary:
          'El botón CTA "Explorar Colección" no desplaza suavemente al catálogo',
        steps:
  ...
  if (await cartBtn.isVisible().catch(() => false)) {
    ...
    if (isCartVisible) {
      const emptyCartBtn = cartSheet.locator(
        'button:has-text("Explorar Colección")',
      );
      if (await emptyCartBtn.isVisible().catch(() => false)) {
        ...
        if (isOpenAfterClick) {
          recordIssue({
            ...
            summary:
              'El botón "Explorar Colección" dentro del carrito vacío no cierra el modal',
            steps:
              '1. Abrir carrito vacío\n2. Hacer click en "Explorar Colección"',
  ...
  if (await firstProductCard.isVisible().catch(() => false)) {
    ...
    if (!(await productModal.isVisible().catch(() => false))) {
      recordIssue({
        ...
        summary:
          "Al hacer click en la card de un producto no se abre el modal de detalles",
        steps: "1. Hacer click en la imagen/card de un producto en el catálogo",
        expected:
          "Se abre el modal ProductDetailModal con información del producto",
  ...
  if (
    !adminHeading.toLowerCase().includes("camila") &&
    !adminHeading.toLowerCase().includes("accede") &&
    !adminHeading.toLowerCase().includes("panel")
  ) {
    recordIssue({
      id: "BUG-010",
      component: "Admin Route",
      summary:
        "La ruta /admin/login no muestra el formulario de inicio de sesión de administrador esperado",
  ...
  if (hasHorizontalScroll) {
    recordIssue({
      ...
      summary:
        "Desbordamiento horizontal (overflow-x) detectado en vista móvil (375px)",
      steps:
        "1. Redimensionar viewport a 375px de ancho\n2. Inspeccionar scroll horizontal",
  ...
  console.log("\n" + "=".repeat(60));
  console.log(
    `📋 AUDITORÍA QA COMPLETADA. Se encontraron ${issues.length} observaciones.`,
  );
  console.log("=".repeat(60));

  // Guardar en JSON
  fs.writeFileSync(
    path.join(REPORT_DIR, "issues.json"),
