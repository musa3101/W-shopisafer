...
async function run() {
  try {
    ...
    const modalStatus = await page.evaluate(() => {
      ...
      const innerScrollRect = innerScrollDiv
        ? innerScrollDiv.getBoundingClientRect()
        : null;
      ...
      return {
        ...
        scrollDivScrollHeight: innerScrollDiv
          ? innerScrollDiv.scrollHeight
          : null,
        scrollDivIsScrollable: innerScrollDiv
          ? innerScrollDiv.scrollHeight > innerScrollDiv.clientHeight
          : false,
      };
    });

    console.log(
      "Mobile Product Modal status:",
      JSON.stringify(modalStatus, null, 2),
    );
