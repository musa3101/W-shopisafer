...
test.describe('01 — Homepage & Navegación', () => {
  ...
  test('carga la página principal con título SEO correcto', async ({ homePage }) => {
  ...
  test('el Hero section es visible con título principal', async ({ homePage }) => {
  ...
  test('el CTA "Explorar Colección" es visible y clickable', async ({ homePage }) => {
  ...
  test('la navbar tiene logo, carrito y menú de usuario', async ({ homePage }) => {
    // Logo
  ...
  test('el footer es visible con información de contacto', async ({ homePage }) => {
    // Scroll al final
